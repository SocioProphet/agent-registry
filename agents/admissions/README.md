...
**Owner sign-off (2026-08-02).** Lord Michael (owner) approved all 9. Each is
now `admission_status: admitted`, `authority.granted: true`, with
...
and — for this destructive/offensive class — a **containment ref**
(`policy://containment/…`), plus `authority.sign_off` recording the approval.
...
NOT hand it unconditional authority. The per-action authority state and the
fail-closed authorize gate still govern each individual action dimension, and
every destructive verb is bounded by its containment ref. The permanent
fail-closed teeth remain: a capability-bearing agent with **no** admission entry
(`unadmitted-capability-agent.invalid.json`) is still denied
